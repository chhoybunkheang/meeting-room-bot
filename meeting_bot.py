# meeting_bot_final.py
"""
Meeting Room Bot (Final Version)
✅ Sorted schedule old → new for all announcements
✅ /schedule removed
✅ Auto webhook clear (fixes 'Conflict: terminated by other getUpdates')
✅ Proper crash alert to admin
✅ For Render or local run
"""

import os, json, re, asyncio, pytz
...
# ===================== HELPERS =====================
def now_phnom_penh(): return datetime.now(ZoneInfo(TZ))
...
def is_overlapping(s1,e1,s2,e2): return not (e2 <= s1 or s2 >= e1)
...
def sort_records_old_to_new(records):
    def key(r):
        try:
            ...
            return (d, t)
        except: return (datetime.max, datetime.max)
    ...
    for r in sheet.get_all_records():
        if r.get("Date")==date_str:
            try:
                ...
                if is_overlapping(time_to_minutes(es), time_to_minutes(ee), ns, ne): return "overlap"
            except: continue
...
async def start(update, context):
    u=update.message.from_user; log_user_action(u,"/start")
...
async def book(update, context):
    ...
    await update.message.reply_text("📅 Enter date (e.g. 30/10 or 30/10/2025):"); return DATE

async def get_date(update, context):
    t=update.message.text.strip()
    if not re.match(r"^\d{1,2}/\d{1,2}(/?\d{2,4})?$",t):
        await update.message.reply_text("❌ Format: DD/MM or DD/MM/YYYY"); return DATE
    if len(t.split("/"))==2: t=f"{t}/{datetime.now().year}"
    d=dateparser.parse(t,settings={"DATE_ORDER":"DMY"})
    if not d or d.date()<datetime.now().date():
        await update.message.reply_text("⚠️ Invalid or past date."); return DATE
    context.user_data["date"]=d.strftime("%d/%m/%Y")
    await update.message.reply_text("⏰ Enter time range (e.g. 14:00-15:00):"); return TIME

async def get_time(update, context):
    time_input=update.message.text.strip(); u=update.message.from_user; date_str=context.user_data["date"]
    if not re.match(r"^\d{1,2}:\d{2}\s*-\s*\d{1,2}:\d{2}$",time_input):
        await update.message.reply_text("❌ Format: HH:MM-HH:MM"); return TIME
    s,e=[t.strip() for t in time_input.split("-")]
    try:
        if datetime.strptime(e,"%H:%M")<=datetime.strptime(s,"%H:%M"):
            await update.message.reply_text("⚠️ End must be after start."); return TIME
    except: return TIME
    res=save_booking(date_str,time_input,u.first_name,u.id)
    if res=="overlap": await update.message.reply_text("⚠️ Overlaps another booking."); return TIME
    if res=="invalid": await update.message.reply_text("❌ Invalid time."); return TIME
    ...
    for r in recs: msg+=f"{r['Date']} | {r['Time']} | {r['Name']}\n"
    await context.bot.send_message(GROUP_CHAT_ID,msg,parse_mode="Markdown")
    return ConversationHandler.END

# --- CANCEL ---
async def cancel(update, context):
    u=update.message.from_user; log_user_action(u,"/cancel")
    recs=sheet.get_all_records()
    own=[(i+2,r) for i,r in enumerate(recs) if str(r.get("TelegramID"))==str(u.id)]
    if not own: await update.message.reply_text("❌ No bookings."); return ConversationHandler.END
    msg="🗓 *Your Bookings:*\n\n"+'\n'.join([f"{i+1}. {r['Date']} | {r['Time']}" for i,(_,r) in enumerate(own)])
    msg+="\n\nReply with number to cancel:"; await update.message.reply_text(msg,parse_mode="Markdown")
    context.user_data["user_bookings"]=own; return CANCEL_SELECT

async def delete_booking_by_number(update, context):
    try: c=int(update.message.text)
    except: await update.message.reply_text("❌ Invalid number."); return CANCEL_SELECT
    u=update.message.from_user; b=context.user_data.get("user_bookings",[])
    if not (1<=c<=len(b)): await update.message.reply_text("❌ Invalid choice."); return CANCEL_SELECT
    idx,data=b[c-1]; sheet.delete_rows(idx)
    recs=sort_records_old_to_new(sheet.get_all_records())
    msg=(f"❌ {u.first_name} *CANCEL* booking:\n📅 {data['Date']} | ⏰ {data['Time']}\n\n📋 *Updated Schedule:*\n"+
         ''.join([f"{r['Date']} | {r['Time']} | {r['Name']}\n" for r in recs]))
    await context.bot.send_message(GROUP_CHAT_ID,msg,parse_mode="Markdown")
    await update.message.reply_text("✅ Booking canceled."); return ConversationHandler.END

# --- END MEETING ---
async def end_meeting(update, context):
    u=update.message.from_user; log_user_action(u,"/end")
    recs=sheet.get_all_records()
    own=[(i+2,r) for i,r in enumerate(recs) if str(r.get("TelegramID"))==str(u.id)]
    if not own: await update.message.reply_text("❌ No meetings."); return
    ...
    for i,r in own:
        try:
            ...
            if sdt<=now<=edt+timedelta(minutes=30):
                ...
                msg=f"🏁 *Meeting Ended!*\n👤 {u.first_name}\n📅 {r['Date']} | ⏰ {r['Time']}"
                await context.bot.send_message(GROUP_CHAT_ID,msg,parse_mode="Markdown")
                await update.message.reply_text("✅ Meeting ended."); return
        except: continue
...
async def auto_cleanup(update=None, context=None):
    ...
    recs=sheet.get_all_records(); rem=[]; kept=[]
    for r in recs:
        try:
            s,e=r["Time"].split("-")
    ...
    if rem:
        sheet.clear(); sheet.update("A1",[["Date","Time","Name","TelegramID"]]+[[r["Date"],r["Time"],r["Name"],r["TelegramID"]] for r in kept])
        msg="🧹 *Expired Meetings Removed:*\n"+''.join([f"• {r['Date']} | {r['Time']} | {r['Name']}\n" for r in rem])
        if kept:
            msg+="\n📋 *Updated Schedule (old → new):*\n"
            for r in sort_records_old_to_new(kept): msg+=f"{r['Date']} | {r['Time']} | {r['Name']}\n"
        else: msg+="\n✅ No meetings left."
        await context.bot.send_message(GROUP_CHAT_ID,msg,parse_mode="Markdown")
...
async def upload_doc_start(update, context):
    ...
    await update.message.reply_text("📤 Send document:",reply_markup=ReplyKeyboardRemove()); return UPLOAD_DOC

async def receive_document(update, context):
    doc=update.message.document
    if not doc: await update.message.reply_text("⚠️ Send a file."); return UPLOAD_DOC
    p=os.path.join("docs",doc.file_name)
    await (await doc.get_file()).download_to_drive(p)
    await update.message.reply_text(f"✅ Uploaded {doc.file_name}"); return ConversationHandler.END

async def docs_menu(update, context):
    files=[f for f in os.listdir("docs") if f!=".keep"]
    if not files: await update.message.reply_text("📂 No documents."); return ConversationHandler.END
    keys=[[f"📄 {f}"] for f in files]
    await update.message.reply_text("📁 Choose document:",reply_markup=ReplyKeyboardMarkup(keys,one_time_keyboard=True)); return DOC_SELECT

async def send_selected_doc(update, context):
    f=update.message.text.replace("📄 ","").strip(); p=os.path.join("docs",f)
    if not os.path.exists(p): await update.message.reply_text("⚠️ File not found."); return ConversationHandler.END
    await update.message.reply_document(InputFile(open(p,"rb"),filename=f),caption=f"📘 {f}"); return ConversationHandler.END

# --- ADMIN ---
async def announce(update, context):
    if update.message.from_user.id!=ADMIN_ID:
        await update.message.reply_text("🚫 Unauthorized."); return ConversationHandler.END
    await update.message.reply_text("📝 Type announcement:"); return ANNOUNCE_MESSAGE

async def send_announcement(update, context):
    msg=update.message.text.strip()
    if not msg: await update.message.reply_text("⚠️ Empty message."); return ANNOUNCE_MESSAGE
    await context.bot.send_message(GROUP_CHAT_ID,f"📢 *Announcement:*\n\n{msg}",parse_mode="Markdown")
    await update.message.reply_text("✅ Announcement sent."); return ConversationHandler.END

async def stats(update, context):
    if update.message.from_user.id!=ADMIN_ID:
        await update.message.reply_text("🚫 Unauthorized."); return
    recs=stats_sheet.get_all_records()
    if not recs: await update.message.reply_text("📊 No data."); return
    summary={}
    for r in recs:
        n,c,t=r["Name"],r["Command"],r["DateTime"]
        s=summary.setdefault(n,{"total":0,"actions":{}, "last":t})
        s["total"]+=1; s["actions"][c]=s["actions"].get(c,0)+1; s["last"]=t
    msg="📊 *User Activity:*\n\n"
...
async def notify_admin(bot,msg):
    try: await bot.send_message(ADMIN_ID,f"⚠️ [Bot Alert]\n\n{msg}")
