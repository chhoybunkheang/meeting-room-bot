...
async def notify_admin_startup(bot: Bot):
    ...
    try:
        now = datetime.now(ZoneInfo("Asia/Phnom_Penh")).strftime("%d/%m/%Y %H:%M:%S")
        message = (
            f"🟢 *Meeting Room Bot Started Successfully!*\n\n"
            f"🕒 Time: {now}\n"
            f"✅ The bot is now running on Render.\n"
            f"If you didn’t restart it manually, this may indicate an automatic redeploy or recovery."
        )
        await bot.send_message(chat_id=ADMIN_ID, text=message, parse_mode="Markdown")
        print("✅ Startup notification sent to admin.")
    except Exception as e:
        print(f"⚠️ Could not notify admin on startup: {e}")


async def notify_admin_shutdown(bot: Bot):
    """Notify admin when the bot stops or redeploys."""
    try:
        now = datetime.now(ZoneInfo("Asia/Phnom_Penh")).strftime("%d/%m/%Y %H:%M:%S")
        message = (
            f"🔴 *Meeting Room Bot Stopped!*\n\n"
            f"🕒 Time: {now}\n"
            f"⚠️ The service was stopped or redeployed on Render."
        )
        await bot.send_message(chat_id=ADMIN_ID, text=message, parse_mode="Markdown")
        print("🔴 Shutdown notification sent to admin.")
    except Exception as e:
        print(f"⚠️ Could not notify admin on shutdown: {e}")


async def notify_admin_crash(bot: Bot, error_text: str):
    """Notify admin when the bot crashes unexpectedly."""
    try:
        now = datetime.now(ZoneInfo("Asia/Phnom_Penh")).strftime("%d/%m/%Y %H:%M:%S")
        message = (
            f"🚨 *Meeting Room Bot CRASHED!*\n\n"
            f"🕒 Time: {now}\n"
            f"❗ Error: `{error_text}`\n\n"
            f"Please check Render logs for details."
        )
        await bot.send_message(chat_id=ADMIN_ID, text=message, parse_mode="Markdown")
        print("🚨 Crash notification sent to admin.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    try:
        ...
        async def on_startup(application):
            try:
                await notify_admin_startup(application.bot)
            except Exception as e:
                print(f"⚠️ Startup notify failed: {e}")

        async def on_shutdown(application):
            try:
                await notify_admin_shutdown(application.bot)
            except Exception as e:
                print(f"⚠️ Shutdown notify failed: {e}")

        app.post_init = on_startup
        app.post_shutdown = on_shutdown

        # --- Initialize job queue (if not already) ---
        job_queue = getattr(app, "job_queue", None)
        if not job_queue:
            try:
                job_queue = JobQueue()
        ...
        # --- Register command handlers, conversations, etc. ---
        # (You already have this above; do not duplicate)
        # Just make sure your add_handler() lines remain above this main section.

        # --- Auto cleanup every 1 hour ---
        job_queue.run_repeating(auto_cleanup, interval=3600, first=10)
        print("🕒 Auto-cleanup scheduled every 1 hour.")

        print("✅ Meeting Room Bot is starting (polling mode)...")
        app.run_polling()

    except Exception as e:
        # 🔥 Handle crash safely even if loop is closed
        print(f"❌ BOT ERROR: {e}")
        ...
        try:
            ...
            bot = Bot(token=TOKEN)
            loop.run_until_complete(notify_admin_crash(bot, str(e)))
